...
- 从 `main` 拉取最新 `v1.51` 后，在最新主线上移植 AIEP 课程解析。
- MSc(AIEP) 英文授课 10 门课程补充选课建议、适合背景、学习收获、就业连接和整理依据。
- 课程库列表展示 AIEP 选课建议摘要，课程详情页展示结构化“选课解析”。
- 课程搜索纳入选课建议、适合背景、学习收获和就业连接字段。
- README 和课程数据检测报告同步到 `v1.52`。

测试：

- `npm run validate:courses` 通过。
- `npm run build` 通过。
