...
v1.24 schema 包含 `otter_posts` 表、`lower(username)` 唯一索引和 `service_role` 表权限。即使之前已经运行过旧版 schema，也需要重新运行当前 `supabase/schema.sql`，否则管理员新增/编辑生活内容会因为表不存在而无法持久化，或注册用户名唯一性无法由数据库兜底。
