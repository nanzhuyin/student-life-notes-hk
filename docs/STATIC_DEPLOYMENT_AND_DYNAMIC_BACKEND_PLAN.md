...
- 当前模式：内测前端 + 注册接口接入准备模式
- 部署分支：`main`

GitHub Pages 页面已明确标注为内测版本。用户可以正常浏览课程库、生活内容、隐私政策、学术诚信说明和收藏；注册入口已开放，配置 `VITE_API_BASE_URL` 后会写入后端，未配置时只保存本机用户状态。站内信箱和在线投稿处理仍在灰度测试。内测期间如需投稿、纠错或联系管理人员，先通过微信群沟通。
...
重新部署后，前端会在构建时读取 `VITE_API_BASE_URL`。确认 Render + Supabase 均验证通过后，注册会写入服务端；统计、投稿、管理端和站内信箱再按灰度计划逐步打开。
