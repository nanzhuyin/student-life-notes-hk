...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.60 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容、AI 专业推荐、AI 课程顾问、邮箱注册登录和联系作者信箱；游客可以浏览课程和生活内容，注册用户和管理员才可以使用 AI 专业推荐分析、AI 课程顾问、发送建议或联系作者。
...
- AI 专业推荐：右侧悬浮入口开放专业推荐助手；游客只能查看页面说明，注册用户和管理员才可以提交 AI 分析请求，后端接口也会拒绝未带登录凭证的请求；推荐结果课程块使用深色文字，保证可读。
- AI 推荐表单：本科专业为必填；只有选择 Doctor 或博士目标专业时才要求硕士专业，其余背景信息为选填；新增“其他情况”选填输入，已选择目标专业时不再重复选择目标学位层级。
- AI 专业推荐学校过滤：专业推荐页的已选目标专业下拉会跟随当前学校，只显示当前学校已收录专业；岭南页面不再显示 EdUHK 专业，EdUHK 页面不再显示岭南专业。
- AI 推荐知识库：岭南专业库新增官网可见的职业方向 / 毕业去向文本结构化字段；官网没有完整按学年毕业去向时会标记信息不足，推荐结果至少返回 3 个候选专业并展示未来去向参考。
- AI 课程顾问：课程详情页新增课程顾问面板，学生可填写年龄、专业/工作背景、是否应届生、职业目标和具体问题；系统基于当前课程库先生成本地规则建议，登录用户和管理员可通过后端代理调用 DeepSeek 生成个性化选课分析，游客不能直接调用后端模型。
- 岭南官网课程描述：重新解析官网课程描述展开块和课程页，知识库新增 53 条官网英文课程描述、53 条中文机器翻译和学生视角课程指南；前端课程详情页可展示匹配到的 49 条官网中文描述、英文原文、适合背景、建议深化、能力产出、就业 / 转型衔接和课前准备建议；费用、方向标题等非单门课程说明不会作为课程展示。
- 管理端统计：修复 Supabase 单次读取默认最多 1000 条导致事件数总是显示 1000 的问题；管理端现在显示服务端真实事件总数，并注明当前统计实际加载范围。
- 岭南生活内容：新增 6 篇美食整理和 6 篇出行景点整理，覆盖富泰、兆康、红桥、新墟、屯门市中心、屯门公园、黄金海岸、青山/屯门径和妙法寺等学生常用路线。
- 岭南路线信息：岭南可见的 15 篇美食和景点内容均补充“从岭南大学过去”的路线、交通工具、预计时间和出发提醒。
- 房源小区图：新增 NOVO LAND、兆康苑、菁隽 3 张可核验授权的本地图片，并在 `public/images/properties/CREDITS.md` 记录来源；未确认开放图源的小区继续使用带小区名的本地占位图。
- AI 推荐日志：Supabase 暂未执行新版 schema 时，后端会兼容旧表结构写入日志，不影响用户获得推荐结果。
- 登录状态：管理员 token 改为签名 token，并自动清理旧版过期登录态，避免刷新后显示已登录但 AI 不可用。
...
- AIEP 课程解析：MSc(AIEP) 英文授课 10 门课程已加入选课建议、适合背景、学习收获、就业连接和整理依据，帮助不同背景学生判断必修课、选修课和项目课的学习路径。
...
管理员新增和编辑生活内容会写入后端 `otter_posts`。部署 v1.60 后需要在 Supabase SQL Editor 重新运行 `supabase/schema.sql`，创建或更新 `otter_posts`、`programmes`、`recommendation_logs` 等表、用户名唯一索引、`service_role` 表权限和其他索引。
