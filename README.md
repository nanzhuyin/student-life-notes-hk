...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.23 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容浏览、邮箱注册登录和联系作者信箱。
...
管理员新增和编辑生活内容会写入后端 `otter_posts`。部署 v1.23 后需要在 Supabase SQL Editor 重新运行 `supabase/schema.sql`，创建 `otter_posts` 表、用户名唯一索引和其他索引。
