...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.59 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容、AI 专业推荐、AI 课程顾问、邮箱注册登录和联系作者信箱；游客可以浏览课程和生活内容，注册用户和管理员才可以使用 AI 专业推荐分析、AI 课程顾问、发送建议或联系作者。
...
管理员新增和编辑生活内容会写入后端 `otter_posts`。部署 v1.59 后需要在 Supabase SQL Editor 重新运行 `supabase/schema.sql`，创建或更新 `otter_posts`、`programmes`、`recommendation_logs` 等表、用户名唯一索引、`service_role` 表权限和其他索引。
