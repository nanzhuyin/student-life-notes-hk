...
这是一个非官方、个人/学生自发维护的香港学习生活信息静态网站。v1.08 已支持香港教育大学 EdUHK 和岭南大学 LU 两个学校平台：课程库、收藏和生活内容都会按当前学校分别显示，并提供管理端本机浏览统计与导出。
...
项目当前不接入后端服务或平台专用 API。管理端账号校验和统计事件都运行在浏览器本机；如需查看所有用户的真实数据，需要接入后端登录、权限校验和服务端统计。
