...
这是一个非官方、个人/学生自发维护的香港学习生活信息静态网站。v1.07 已支持香港教育大学和岭南大学两个学校平台：课程库、收藏和生活内容都会按当前学校分别显示，并新增管理端本机浏览统计与导出。
...
- 管理视角：改为管理员账号 + 强密码登录，支持查看本机总浏览量、EDU / LU 浏览量、停留时间、功能打开排行和最近事件。
- 统计导出：管理端可导出 JSON / CSV 数据，便于后续整理成报表。
...
项目当前不接入后端服务或平台专用 API。管理端账号校验和统计事件都运行在浏览器本机；如需查看所有用户的真实数据，需要后续接入后端登录、权限校验和服务端统计。
