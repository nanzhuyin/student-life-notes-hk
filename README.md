...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.30 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容、邮箱注册登录和联系作者信箱；游客可以浏览课程和生活内容，注册用户才可以发送建议或联系作者。
...
- 用户登录：顶部导航开放“登录”，注册页提供“已有账号，直接登录”入口；可用邮箱或用户名登录。
- 访问门槛：首页确认隐私与学术诚信说明后先进入注册/登录；注册或登录成功后才进入课程库、收藏和生活内容。
...
- 管理端入口：顶部导航不会向普通用户公开显示“管理视角”；管理员登录后才显示管理入口，并可从普通页面退出管理状态。
...
- 联系作者：当前内测期可通过底部联系作者信箱提交纠错、补充资料和使用问题；内容不会直接公开。
...
管理员新增和编辑生活内容会写入后端 `otter_posts`。部署 v1.30 后需要在 Supabase SQL Editor 重新运行 `supabase/schema.sql`，创建 `otter_posts` 表、用户名唯一索引、`service_role` 表权限和其他索引。
