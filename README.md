...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.22 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容浏览、邮箱注册和联系作者信箱。
...
- 用户注册：开放邮箱、用户名和密码注册；后端检查邮箱格式和可用邮箱域名，不要求邮箱二次验证。
- 联系作者信箱：页面底部开放私信反馈入口，可提交联系作者、内容纠错、资料补充和使用问题。
- 回执查询：填写邮箱提交后，可以在底部“我的回执”查看管理员处理状态和回复。
- 动态功能边界：公开投稿、评论和社区功能不开放，内容仍由管理员确认后整理发布。
- 管理员发布：管理员登录后可在各生活板块直接新增内容，并按板块选择对应标签和补充字段。
- 管理员编辑：管理员可在板块列表或详情页直接编辑已有内容；编辑静态内容时以后端覆盖层保存。
- 共享范围：管理员发布时可选择当前学校、两校共享、仅教育大学或仅岭南大学。
- 图片策略：当前先填写图片 URL；图片文件建议放对象存储或 CDN，数据库只保存 URL。
...
- 投稿建议：当前内测期可通过底部联系作者信箱提交；内容不会直接公开。
...
- 注册入口：顶部导航开放“注册”，注册成功后保存当前浏览器用户身份。
- 登录界面：当前没有单独登录页；已有邮箱再次注册时，如果密码正确，会更新用户名和学校选择。
...
前端通过 `VITE_API_BASE_URL` 接入后端 API。配置后端地址后，注册和联系作者信箱会写入服务端；未配置时使用本机备用数据。公开投稿、评论和社区功能不开放。

管理员新增和编辑生活内容会写入后端 `otter_posts`。部署 v1.22 后需要在 Supabase SQL Editor 重新运行 `supabase/schema.sql`，创建 `otter_posts` 表和索引。
