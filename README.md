...
Otter 是一个非官方、个人/学生自发维护的香港学习生活信息网站。v1.20 是内测版本，当前开放两个学校平台的课程库、收藏、生活内容浏览和邮箱注册。
...
- 动态功能边界：站内信箱和在线投稿处理仍在灰度测试，反馈先通过微信群联系管理人员。
...
- 投稿建议：当前内测期通过微信群联系管理人员；线上表单和站内信箱仍在灰度测试。
...
前端通过 `VITE_API_BASE_URL` 接入后端 API。配置后端地址后，注册会写入服务端；未配置时仅使用本机备用用户状态。站内信箱和在线投稿处理仍在灰度测试。
